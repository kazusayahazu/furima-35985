class PurchaseAddress
  ...
  attr_accessor :user_id, :item_id, :postal_code, :shipment_source_id, :municipalities, :address, :building_name,
                :telephone_number, :token

  with_options presence: true do
    validates :user_id, :item_id, :municipalities, :address, :token
    validates :postal_code, format: { with: /\A\d{3}-\d{4}\z/, message: 'は無効です ハイフン(-)を入力してください' }
  ...
  def save
    ...
    Address.create(postal_code: postal_code, shipment_source_id: shipment_source_id, municipalities: municipalities,
